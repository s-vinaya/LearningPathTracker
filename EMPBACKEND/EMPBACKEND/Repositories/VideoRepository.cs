...
namespace EMPBACKEND.Repositories
{
    public class VideoRepository : IVideoRepository
    {
        ...
        {
            return await _context.Videos.Include(v => v.Course).ToListAsync();
        }

        public async Task<Video?> GetByIdAsync(int id)
        {
            return await _context.Videos.Include(v => v.Course).FirstOrDefaultAsync(v => v.Id == id);
        }

        public async Task<IEnumerable<Video>> GetByCourseIdAsync(int courseId)
        {
            return await _context.Videos.Where(v => v.CourseId == courseId).Include(v => v.Course).ToListAsync();
        }
